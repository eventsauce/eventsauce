...
/**
 * Describes a pending event for an aggregate that has not been commited to the repository (yet).
 **/
class PendingEvent {
  /**
   * Initialize a new instance of the PendingEvent.
  ...
  constructor(eventType, eventData) {
    if (validationHelpers.isNullOrWhitespace(eventType)) {
      throw new Error('The eventType for a PendingEvent cannot have a name that is null or whitespace only.');
    }
